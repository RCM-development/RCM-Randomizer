...
- **Skills** (`SkillInjector.cs`): a catalog of active skills injected at spawn onto units that have none, priced into the roll budget. The economy harvesters always roll a replacement for their stock skill, weighted towards mining skills; nobody else's own skill is touched (specialists only with `Skills.ReplaceSpecialistSkills`). A skill that cannot build its actions, or that only sets a status flag nothing listens to, is not offered. Every cast logs one line.
...
- Also: enemy stat rolls that escalate per level (`EnemyRolls.cs`), captured enemy turrets as blueprints, a seeded engineer trait (`EngineerTraits.cs`), shop sales and rarity bumps (`ShopTweaks.cs`), support-aura variance (`AuraTweaks.cs`), three-tier veterancy (`Veterancy.cs`): kill credits on a steep price (6/18/48), 15 percent per tier, the veteran icon tinted bronze, silver or gold; specialist hack trees refitted to a swapped starter skill (`SpecialistHacks.cs`, opt-in); the engineer career (`EngineerVeterancy.cs`): ranks from building, double bonus, one hack per rank, carried through the run; Titans (`Titans.cs`): seeded super units and turrets as appended unit + foundry rows, id stamped before `Init`, plus late-run enemy Titans.
